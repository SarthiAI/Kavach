...
`kavach-http` contains a framework-agnostic `HttpGate`, a Tower `Layer` for Axum, and an Actix adapter. The crate compiles and has internal tests, but it has not yet been exercised by the consumer-validation harness the rest of the Kavach surface has passed through.
